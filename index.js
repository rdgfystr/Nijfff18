...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/NazrulFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/NazrulFca.json", JSON.stringify(global.Fca.Data.ObjNazrul, null, "\t"));
        process.exit(1);
    }
...
try {
    ...
    global.Fca.Require.logger.Error('Detect Your NazrulFca Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/NazrulFca.json", JSON.stringify(global.Fca.Data.ObjNazrul, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/NazrulFca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                Data_Setting[i] = global.Fca.Data.ObjNazrul[i];
        ...
        for (let i of Object_Fca) {
            const All_Paths = utils.getPaths(global.Fca.Data.ObjNazrul[i]);
            const Mission = { Main_Path: i, Data_Path: All_Paths }
            for (let i of Mission.Data_Path) {
                if (Data_Setting[Mission.Main_Path] == undefined) {
                    Data_Setting[Mission.Main_Path] = global.Fca.Data.ObjNazrul[Mission.Main_Path];
                    global.Fca.Require.fs.writeFileSync(process.cwd() + "/NazrulFca.json", JSON.stringify(Data_Setting, null, "\t"));      
                }
                ...
                if (User_Data_Type == "Number") {
                    ...
                    const Mission_Obj = utils.getData_Path(global.Fca.Data.ObjNazrul[Mission.Main_Path], Mission_Path, 0);
    ...
    global.Fca.Require.Nazrul = Data_Setting;
}
catch (e) {
    console.log(e);
...
module.exports = function(loginData, options, callback) {
 ...
 require('./Extra/Database');
    ...
    /*
    return got.get('https://github.com/nazrul6x/FB-BOT/raw/main/InstantAction.json').then(async function(res) {
